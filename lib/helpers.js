...
exports.event = function (title, options) {
    ...
    if (context.who) {
        ...
        switch (context.who) {
		case 'elizabeth':
			context.who = {name: 'Elizabeth', abbrv: 'EJ'};
			break;
		case 'jeff':
			context.who = {name: 'Jeff', abbrv: 'JM'};
			break;
        }
    }

    if (context.classNames) {
	classNames = classNames.concat(context.classNames);
    }

    var styleString = '';
    if (context.style) {
	styleString = context.style;
    }
    ...
    output = '<li class="' + classNames.join(' ') + '" style="' + styleString + '">\n' +
                options.fn(context) + '\n' +
             '</li>\n';
